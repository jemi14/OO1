...
/*----------------------------------------------------------------------------------*/
			/*	public static boolean esValidoCUIL() throws Exception {
					boolean esValido = false; 
					
					//En este caso la dimension del cuil es 11 pero hago el codigo prolijo
					int dimensionCuil = cuil.length();
					
					//Genero el array
					int arrayCuil[] = new int [dimensionCuil];
					
					//Guardo cada coordenada del cuil en un array :) 
					for (int i=0; i<dimensionCuil;i++){
						int cuilAux = Integer.parseInt(cuil.substring(i,i+1));
					     arrayCuil[i] = cuilAux;
					}
					
					//Este es el codigo de validacion, podria haberlo metido en el for, pero 
					//prefiero diferenciar este digito
					int cuilAValidar = Integer.parseInt(cuil.substring(10,11));
					
					//Array para el multiplo
					final int ARRAYMULTIPLOS[] = {5,4,3,2,7,6,5,4,3,2};
					
					int i; 
					int valor1 = 0;
					for(i=0;i<10;i++){  
						int multiplicacion = arrayCuil[i]*ARRAYMULTIPLOS[i];
						valor1 = valor1 + multiplicacion; 
						}//Cierra el for
					
					int valor2 = valor1%11;
					
					int valor3 = 11 - valor2;
					
					if (valor3 == cuilAValidar){
						esValido = true; 
						
					}//Cierra if
					
					if (esValido==false) {
						throw new Exception("Error: Cuil invalid");
					}
